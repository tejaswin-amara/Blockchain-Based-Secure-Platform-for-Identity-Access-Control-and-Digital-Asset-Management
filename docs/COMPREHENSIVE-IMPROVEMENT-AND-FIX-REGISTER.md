...
| A disposable Hardhat JSON-RPC verifier regression deploys the canonical contract, verifies direct-RPC owner/asset/metadata/code-hash evidence, and proves bytecode-hash mismatch denial; the genuine PostgreSQL suite covers a same-idempotency-key race plus uncertain-event replay and deterministic reconciliation finding persistence; the static console has a project-owned bundle-budget gate. | `scripts/tests/test_verifier_e2e.mjs`, `scripts/tests/test_persistence_postgres.py`, `scripts/check_web_bundle_budget.mjs`, CI workflow, `docs/OPTIMIZATION-ADOPTION.md` | Local/disposable evidence only; approved network/finality, durable worker, live identity/auth, Docker-capable runtime health, manual accessibility, and production operations remain required. |
